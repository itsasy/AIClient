...
class AgentLoader:
    ...
    def load_module(self, module_path: str) -> None:
        ...
        try:
            ...
            agents = self.discover(module)
            for agent_class in agents:
                self.register(agent_class)
            self.loaded_modules.add(module_path)
        except Exception:
            logger.exception("Error cargando Agent module=%s", module_path)

    def discover(self, module) -> list[type[Agent]]:
    ...
    def register(self, agent_class: type[Agent]) -> None:
        ...
        if not name:
            logger.warning("Agent sin name=%s", agent_class)
            return

        try:
            self.registry.register(name=name, factory=agent_class)
            self.loaded_agents.add(name)
            logger.info("Agent registrado=%s", name)
        except ValueError:
            logger.debug("Agent ya registrado=%s", name)
