...
class ContinuousLearner:
    ...
    def __init__(self):
        ...
        logger.info("ContinuousLearner inicializado (backend: %s)", self.backend)
    ...
    def extract_and_learn(self, user_query: str, assistant_response: str) -> bool:
        ...
        for pattern in learn_patterns:
            ...
            if match:
                ...
                return self._learn_from_text(raw_text, user_query)

        return False

    def _learn_from_text(self, text: str, original_query: str) -> bool:
...
"""
        try:
            provider = ProviderSelector.select(
                task="learn_extraction", skill_name="learning"
            )
            ...
            if key and value:
                self._save_standard(key, value, original_query, text)
    ...
    def _save_standard(self, key: str, value: str, original_query: str, raw_text: str):
        ...
        if backend in ("engram", "both"):
            ...
            self.engram.save(content, tags=tags, source="continuous_learner")
            ...
            self.engram.save(
                context_content,
                tags=["learning_context", "trazabilidad", f"key_{key}"],
    ...
    def get_context(self) -> str:
        ...
        if backend in ("engram", "both"):
            ...
            for m in memories:
                ...
                if content.startswith("Estándar aprendido:"):
                    try:
                        parts = content.replace("Estándar aprendido: ", "").split(" = ")
                        if len(parts) == 2:
        ...
        if backend in ("legacy", "both"):
            legacy_standards = self.standards.list_standards()
            for key, value in legacy_standards.items():
                if key not in seen_keys:
                    lines.append(f"- {key}: {value}")
                    seen_keys.add(key)
    ...
    def learn_direct(self, key: str, value: str, context: str = "") -> bool:
        """Método directo para aprender un estándar sin LLM."""
        if not key or not value:
            return False
        self._save_standard(
            key, value, context or f"Directo: {key} = {value}", f"{key} = {value}"
        )
        return True
